...
var vf = {
	...
	init:function( eyeSide, highChart ){
		...
		this.loadVFdata( '/newblue/js/highchart-demo-js/glaucoma-fields.json' );
	}, 
	
	/**
	user mouseenters SVG plots
	@param $plot - jQuery SVG dom
	@param checkFixed (Boolean) - used by userChangeFixPlot
	**/
	userSelectsColorPlot:function( $plot, checkFixed = true ){
		
		if( vf.user.fixed && checkFixed ) return; // fixed, ignore mouseevents
		
		vf.chartRegression( parseInt( $plot.data( 'id' ) ) );
		
		// update SVG UI
		$plot.addClass('selected-plot');
		
		if(vf.user.$color !== null ){
	...
	/**
	user mouse clicks to fix/unfix SVG plots
	**/
	userChangeFixPlot:function( $plot ){
		var currentID = vf.user.$color.data( 'id' );
		var newID = $plot.data( 'id' );
		var fix = !vf.user.fixed;
		 
		if( fix == false  && currentID !== newID ){
			// the user has click on a different plot to the currently selected one
			// leave it as fixed but update plots
			vf.userSelectsColorPlot( $plot, false );
		} else {
			vf.user.fixed = fix;
			vf.userSelectsColorPlot( $plot );
		}
	},
	
	/**
	using chart to set colour plot date range
	@param x - chart X axis position (snapping)
	**/
